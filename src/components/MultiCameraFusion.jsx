...
function MultiCameraFusion() {
    ...
    const [frontSessionId, setFrontSessionId] = useState(null);
    const [sideSessionId, setSideSessionId] = useState(null);
    ...
    const frontSession = sessions.find(s => s.id === frontSessionId);
    const sideSession = sessions.find(s => s.id === sideSessionId);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button
                        ...
                        disabled={status !== 'Ready' || !frontSession || !sideSession}
                        style={{
                            padding: '8px 16px',
                            ...
                            cursor: 'pointer',
                            fontWeight: 'bold',
                            opacity: (status !== 'Ready' || !frontSession || !sideSession) ? 0.5 : 1
                        }}
                    >
            ...
            <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr 300px', gap: '20px', minHeight: 0 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <select
                            value={frontSessionId || ''}
                            onChange={(e) => setFrontSessionId(Number(e.target.value))}
                            style={{ padding: '4px', backgroundColor: '#333', color: 'white', border: '1px solid #555', borderRadius: '4px', maxWidth: '200px' }}
                        >
                            <option value="">Select Front Video...</option>
                            {sessions.map(s => <option key={s.id} value={s.id}>{s.videoName}</option>)}
                        </select>
                    </div>
                    <div style={{ flex: 1, position: 'relative', backgroundColor: '#000', borderRadius: '8px', overflow: 'hidden', border: '2px solid #4da6ff' }}>
                        <VideoPanel
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <select
                            value={sideSessionId || ''}
                            onChange={(e) => setSideSessionId(Number(e.target.value))}
                            style={{ padding: '4px', backgroundColor: '#333', color: 'white', border: '1px solid #555', borderRadius: '4px', maxWidth: '200px' }}
                        >
                            <option value="">Select Side Video...</option>
                            {sessions.map(s => <option key={s.id} value={s.id}>{s.videoName}</option>)}
                        </select>
                    </div>
                    <div style={{ flex: 1, position: 'relative', backgroundColor: '#000', borderRadius: '8px', overflow: 'hidden', border: '2px solid #0a5' }}>
                        <VideoPanel
                            session={sideSession}
                ...
                <div style={{ backgroundColor: '#1e1e1e', borderRadius: '8px', padding: '15px', display: 'flex', flexDirection: 'column', gap: '15px', border: '1px solid #333' }}>
                    ...
                    {fusedScore ? (
                        ...
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666', textAlign: 'center' }}>
                            Select videos and start analysis to see fused 3D results.
                        </div>
                    )}
                </div>
            </div>
